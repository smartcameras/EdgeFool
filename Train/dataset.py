...
class SuDataset(data.Dataset):
    def __init__(self, root, list_path, low_size=64, fine_size=-1, loader=default_loader):
        ...
        gt_root = '/jmain01/home/JAD007/txk02/axs14-txk02/image_enhance/kzhang-cs205-l0-smoothing/ImageNet_val_smooth/'
        image_list = []
            
        with open(list_path) as class_file:
            csv_reader = csv.reader(class_file, delimiter=',')
            for line in csv_reader:
                name = line[0]
                inp_path = root+name[:-4]+'.png'
                gt_path = gt_root+name[:-4]+'.png'
                image_list.append([inp_path,gt_path])
